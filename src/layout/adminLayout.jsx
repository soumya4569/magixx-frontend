...
const AdminLayout = () => {
  ...
  // Global Web Bluetooth Device Chooser & Auto-Match State (Active across POS and all routes)
  const [availableBluetoothDevices, setAvailableBluetoothDevices] = useState([])
  const [showGlobalBluetoothModal, setShowGlobalBluetoothModal] = useState(false)
    ...
    if (window.electronAPI && typeof window.electronAPI.onBluetoothDevices === 'function') {
      const unsubscribe = window.electronAPI.onBluetoothDevices((devices) => {
        ...
        try {
          ...
          if (raw) savedPrinters = JSON.parse(raw)
        } catch (e) {}

        const kotAddr = savedPrinters.kotPrinterAddress
        const kotName = savedPrinters.kotPrinterName
        const billAddr = savedPrinters.billingPrinterAddress
        const billName = savedPrinters.billingPrinterName

        // Instant Auto-Match for saved paired printers
        const autoMatch = list.find((dev) => {
          const id = dev.deviceId || dev.id
          const name = dev.deviceName || dev.name
          return (
            (kotAddr && id === kotAddr) ||
            (billAddr && id === billAddr) ||
            (kotName && name && name.toLowerCase() === kotName.toLowerCase()) ||
            (billName && name && name.toLowerCase() === billName.toLowerCase())
          )
        })

        if (autoMatch && window.electronAPI.selectBluetoothDevice) {
          console.log('[AdminLayout] Auto-selecting matched Bluetooth printer:', autoMatch)
          window.electronAPI.selectBluetoothDevice(autoMatch.deviceId || autoMatch.id)
          setShowGlobalBluetoothModal(false)
          return
        }

        // Otherwise display global chooser modal across POS, Settings, Dashboard, etc.
        setAvailableBluetoothDevices(list)
        setShowGlobalBluetoothModal(true)
      })
      return () => {
  ...
  return (
    <div className="flex h-screen w-full overflow-x-hidden bg-gray-50 text-gray-900">
      ...
      {/* ── Global Web Bluetooth Chooser Modal ── */}
      {showGlobalBluetoothModal && (
          ...
          <div className="w-full max-w-md rounded-2xl border border-zinc-200 bg-white p-6 shadow-2xl animate-in fade-in duration-200">
            ...
            <div className="my-4 max-h-64 overflow-y-auto space-y-2 pr-1">
              {availableBluetoothDevices.length === 0 ? (
                ...
                availableBluetoothDevices.map((dev, idx) => (
                  <div
                  ...
                  >
                    <div className="min-w-0 flex-1 pr-3">
                      ...
                      <p className="text-[10px] text-zinc-400 font-mono truncate mt-0.5">
                        ID: {dev.deviceId || dev.id || 'N/A'}
                      </p>
                    </div>
                    <button
                      onClick={() => handleSelectBluetoothDevice(dev.deviceId || dev.id)}
